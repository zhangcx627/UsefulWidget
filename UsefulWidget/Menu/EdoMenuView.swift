...
let UWPopupViewHideAllNotification = "UWPopupViewHideAllNotification"

typealias UWActionInfo = (title: String, icon: String, enabled: Bool, handler: (() -> ()))

class UWMenuViewTableViewCell: UITableViewCell {
    ...
    func setupContent(title: String?, image: UIImage?, property: UWMenuViewProperty, imgSize: CGSize = .zero) {
...
public typealias UWPopupItemClickedBlock = (_ popupView: UWMenuView, _ index: Int, _ title: String?) -> Void
/// 弹窗状态回调，注意：该回调会走N次
public typealias UWPopupStateBlock = (_ popupView: UWMenuView, _ popupViewState: UWPopupViewState) -> Void
/// 弹窗显示、隐藏回调，内部回调，该回调不对外
public typealias UWPopupShowBlock = (_ popupView: UWMenuView) -> Void
/// 弹窗显示、隐藏回调，内部回调，该回调不对外
public typealias UWPopupHideBlock = (_ popupView: UWMenuView, _ hideWithRemove: Bool) -> Void
/// 弹窗已经显示回调
public typealias UWPopupDidAppearBlock = (_ popupView: UWMenuView) -> Void
/// 弹窗已经隐藏回调
public typealias UWPopupDidDisappearBlock = (_ popupView: UWMenuView) -> Void

open class UWMenuView: UIView, UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource {
    
    /// 外部传入的标题数组
    var actions: [UWActionInfo]? {
        didSet {
            self.setNeedsLayout()
    ...
    /// 保存点击回调
    private var popupItemClickedBlock: UWPopupItemClickedBlock?
    ...
    private var popupStateBlock: UWPopupStateBlock?
    ...
    private var showAnimation: UWPopupShowBlock?
    
    private var hideAnimation: UWPopupHideBlock?
    
    private var popupDidAppearBlock: UWPopupDidAppearBlock?
    private var popupDidDisappearBlock: UWPopupDidDisappearBlock?
    ...
    /// 记录当前弹窗状态
    public var currentPopupViewState: UWPopupViewState = .unKnow {
        willSet {
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.register(UWMenuViewTableViewCell.self, forCellReuseIdentifier: "cellId")
    ...
    /// 1、当外部没有传入该参数时，默认为UIWindow的根控制器的视图，即表示弹窗放在UWPopupSWindow上，此时若UWPopupSWindow.sharedInstance.touchWildToHide = true表示弹窗视图外部可点击；2、当外部传入该参数时，该视图为传入的UIView，即表示弹窗放在传入的UIView上；
    @objc public var attachedView = UWPopupSWindow.sharedInstance.attachView() {
        willSet {
            newValue?.UWMaskView.addSubview(self)
            if let new = newValue as? UIScrollView {
                self.originScrollEnabled = new.isScrollEnabled
            }
        }
    }
    @objc public var vProperty = UWMenuViewProperty() {
        willSet {
            self.attachedView?.UWAnimationDuration = newValue.animationDuration
    ...
    private func setupParams() {
        ...
        UWPopupSWindow.sharedInstance.backgroundColor = UIColor.clear
        
        self.originMaskViewColor = self.attachedView?.UWMaskViewColor ?? .clear
        self.attachedView?.UWMaskView.addSubview(self)
        self.isHidden = true
        
        self.showAnimation = self.customShowAnimation()
        self.hideAnimation = self.customHideAnimation()
        
        NotificationCenter.default.addObserver(self, selector: #selector(notifyHideAll(notification:)), name: NSNotification.Name(rawValue: UWPopupViewHideAllNotification), object: nil)
    }
    ...
    ///   - property: 可设置参数
    /// - Returns: self
    class func menu(_ actions: [UWActionInfo], itemBlock: UWPopupItemClickedBlock? = nil, property: UWMenuViewProperty?, imgSize: CGSize? = .zero) -> UWMenuView {
        
        let popupMenu = UWMenuView()
    ...
    public override init(frame: CGRect) {
        ...
        self.setupParams()
        self.vProperty = UWMenuViewProperty()
    }
...
extension UWMenuView {
    
    private func setupUI(_ actions: [UWActionInfo], itemBlock: UWPopupItemClickedBlock? = nil, property: UWMenuViewProperty?, imgSize: CGSize? = nil) {
    ...
    private func setupFrame(property: UWMenuViewProperty) {
        var tableViewY: CGFloat = 0
    ...
    private func setupMaskLayer(property: UWMenuViewProperty) {
        // 绘制箭头
        if property.popupArrowStyle != .none {
...
extension UWMenuView {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cellId", for: indexPath) as? UWMenuViewTableViewCell {
            var title: String?
            var image: UIImage?
            ...
            let cell = UWMenuViewTableViewCell(style: .default, reuseIdentifier: "cellId")
            var title: String?
            var image: UIImage?
            if let actions = self.actions {
                title = actions[indexPath.row].title
    ...
    func show() {
        ...
        if self.attachedView != nil && self.vProperty.maskViewColor != nil {
            if let maskViewColor = self.vProperty.maskViewColor {
                self.attachedView?.UWMaskViewColor = maskViewColor
            }
        }
        ...
        self.attachedView?.UWAnimationDuration = self.vProperty.animationDuration
        
        if self.attachedView != nil && self.attachedView != UWPopupSWindow.sharedInstance.attachView() {
            if let tap = tapGest {
                tap.isEnabled = true
            } else {
        ...
        if self.attachedView == nil {
            self.attachedView = UWPopupSWindow.sharedInstance.attachView()
        }
        
        self.attachedView?.showUWBackground()
    ...
    @objc func tapGesClick(tap: UITapGestureRecognizer) {
        ...
        if !self.UWBackgroundAnimating, let attachedView = self.attachedView {
            for view: UIView in attachedView.UWMaskView.subviews {
    ...
    func hide() {
        ...
        for tmpView: UIView in UWPopupSWindow.sharedInstance.hiddenViews {
            if tmpView == self {
                if let index = UWPopupSWindow.sharedInstance.hiddenViews.firstIndex(of: tmpView) {
                    UWPopupSWindow.sharedInstance.hiddenViews.remove(at: index)
                }
            }
        }
        
        if UWPopupSWindow.sharedInstance.hiddenViews.isEmpty && UWPopupSWindow.sharedInstance.willShowingViews.isEmpty && self.attachedView?.UWBackgroundAnimating == false {
            self.attachedView?.hideUWBackground()
        }
        ...
        // 还原弹窗弹起时的相关参数
        self.attachedView?.UWMaskViewColor = self.originMaskViewColor
...
extension UWMenuView {
    ...
    /// 显示动画
    ///
    /// - Returns: UWPopupShowBlock
    private func customShowAnimation() -> UWPopupShowBlock {
        
        let popupBlock = { [weak self] (popupView: UWMenuView) in
            ...
            for view in attachedView.UWMaskView.subviews {
                if let view = view as? UWMenuView {
                    if view == strongSelf {
                        view.isHidden = false
            ...
            UWPopupSWindow.sharedInstance.hiddenViews.removeAll()
            UWPopupSWindow.sharedInstance.hiddenViews.append(contentsOf: tmpHiddenViews)
            ...
            strongSelf.attachedView?.UWBackgroundAnimating = true
    ...
    private func showAnimationFinished() {
        ...
        if UWPopupSWindow.sharedInstance.willShowingViews.count > 0 {
            if let willShowingView: UWMenuView = UWPopupSWindow.sharedInstance.willShowingViews.first as? UWMenuView {
                willShowingView.show()
                UWPopupSWindow.sharedInstance.willShowingViews.removeFirst()
            }
        } else {
            self.attachedView?.UWBackgroundAnimating = false
        }
    }
    
    /// 隐藏动画
    ///
    /// - Returns: UWPopupHideBlock
    private func customHideAnimation() -> UWPopupHideBlock {
        
        let popupBlock: UWPopupHideBlock = { [weak self] popupView, isRemove in
            ...
            UIView.animate(withDuration: strongSelf.vProperty.animationDuration, animations: {
                ...
                if isRemove == true {
                    ...
                    if let index = UWPopupSWindow.sharedInstance.hiddenViews.firstIndex(of: strongSelf) {
                        UWPopupSWindow.sharedInstance.hiddenViews.remove(at: index)
                    }
                }
                ...
                DispatchQueue.main.asyncAfter(deadline: .now()+0.0001, execute: {
                    if UWPopupSWindow.sharedInstance.willShowingViews.count > 0 {
                        if let willShowingView: UWMenuView = UWPopupSWindow.sharedInstance.willShowingViews.last as? UWMenuView {
                            willShowingView.show()
                            UWPopupSWindow.sharedInstance.willShowingViews.removeLast()
                        }
                    } else if !UWPopupSWindow.sharedInstance.hiddenViews.isEmpty {
                        if let showView: UWMenuView = UWPopupSWindow.sharedInstance.hiddenViews.last as? UWMenuView {
                            showView.isHidden = false
                            showView.currentPopupViewState = .didAppearAgain
                            UWPopupSWindow.sharedInstance.hiddenViews.removeLast()
                        }
                    }
                ...
                strongSelf.attachedView?.UWBackgroundAnimating = false
    ...
    /// 根据不同状态、动画设置视图的不同约束
    ///
    /// - Parameter constraintsState: UWConstraintsState
    private func setupConstraints(constraintsState: UWConstraintsState) {
        
        let myAlignment: UWPopupCustomAlignment = self.vProperty.popupCustomAlignment
    ...
    private func constraintsBeforeAnimationPosition(make: ConstraintMaker, myAlignment: UWPopupCustomAlignment, myPosition: CGRect) {
    ...
    ///   - make: ConstraintMaker
    ///   - myAlignment: 自定义弹窗校准位置
    private func constraintsBeforeAnimationFrame(make: ConstraintMaker, myAlignment: UWPopupCustomAlignment, myPosition: CGRect) {
...
/// UWMenuView的相关属性，请注意其父类中还有很多公共属性
open class UWMenuViewProperty: NSObject {
    ...
    /// 弹窗的背景色（注意：这边指的是弹窗而不是遮罩层，遮罩层背景色的设置是：UWMaskViewColor）
    ...
    /// 弹窗箭头的样式
    @objc open var popupArrowStyle = UWMenuArrowStyle.none
    /// 弹窗箭头的尺寸
    ...
    // ===== 自定义弹窗（继承UWPopupView）时可能会用到 =====
    
    /// 弹窗校准位置
    @objc open var popupPositionAlignment: CGRect = CGRect.zero
    /// defult from left
    @objc open var popupCustomAlignment: UWPopupCustomAlignment = .right
    /// 弹窗动画类型
    @objc open var popupAnimationType: UWPopupAnimationType = .position
    
    /// 弹窗偏移量
    @objc open var popupViewEdgeInsets = UIEdgeInsets.zero
    /// 遮罩层的背景色（也可以使用UWMaskViewColor），注意：该参数在弹窗隐藏后，还原为弹窗弹起时的值
...
private enum UWConstraintsState: Int {
...
/// - didAppearAgain: 已经显示，其上面遮盖的弹窗消失了（实际上当前状态与UWPopupStateDidAppear状态相同）
@objc public enum UWPopupViewState: Int {
    case unKnow
    case willAppear
    case didAppear
    case willDisappear
...
/// - bottomLeft: 下左
/// - bottomRight: 下右
@objc public enum UWPopupCustomAlignment: Int {
    case left
    case right
}
/// 自定义弹窗动画类型
...
@objc public enum UWPopupAnimationType: Int {
    case position
    case frame
}
...
/// - round: 圆角
/// - triangle: 菱角
@objc public enum UWMenuArrowStyle: Int {
    case none
    case round
    case triangle
}

let UWReferenceCountKey: UnsafeRawPointer? = UnsafeRawPointer.init(bitPattern: "UWReferenceCountKey".hashValue)

let UWBackgroundViewKey: UnsafeRawPointer? = UnsafeRawPointer.init(bitPattern: "UWBackgroundViewKey".hashValue)
let UWBackgroundViewColorKey: UnsafeRawPointer? = UnsafeRawPointer.init(bitPattern: "UWBackgroundViewColorKey".hashValue)
let UWBackgroundAnimatingKey: UnsafeRawPointer? = UnsafeRawPointer.init(bitPattern: "UWBackgroundAnimatingKey".hashValue)
let UWAnimationDurationKey: UnsafeRawPointer? = UnsafeRawPointer.init(bitPattern: "UWAnimationDurationKey".hashValue)
